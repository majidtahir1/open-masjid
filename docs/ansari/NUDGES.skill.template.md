<!--
  ...
  Two modes, two triggers:
    ...
    2. REVIEW (interactive) — the admin says "nudges" / "what's pending";
  ...
  says "nudges" — which PULLS the open items into the live conversation. From
...
text rollup (template + length rule below), ending with "Reply **nudges** if
...
Trigger this whenever the admin asks to see what you've flagged: **"nudges"**,
"pending", "what's waiting", "what did you flag", "review", "anything for me".
...
open items and append "…and more — reply **nudges** for the rest." One message,
...
1. **Replies are only understood inside a review.** Never interpret a stray
   "yes" as acting on a nudge. The admin enters the flow by asking for
   **nudges**; that pulls the items into the conversation so answers have a
   referent.
