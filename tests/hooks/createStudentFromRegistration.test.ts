...
/**
 * Mock payload for the afterChange hook. `form` is what findByID('forms')
 * returns; find/create are spies so we can assert whether students were created.
 */
function makePayload(form: Record<string, unknown>) {
...
describe('createStudentFromRegistration afterChange', () => {
  ...
  it('DOES create students when no payment model is set (undefined → free)', async () => {
    const payload = makePayload({
      schoolRegistration: true,
      registration: { participantModel: 'self' },
    })
