 ...
 *
 * This is a separate migration from 20260430_144311_tenant_billing_fields
 * because Postgres forbids using a newly-added enum value within the same
 * transaction that ALTER TYPE'd it (55P04 "unsafe use of new value").
 * The previous migration adds the 'grandfathered' value to enum_tenants_status;
 * this one runs in a fresh transaction after that commit, so it can use it.
 *
 * Idempotent: safe to re-run. Only updates rows still in the legacy
 * (pending|active) states — never touches tenants already on the new
 * billing lifecycle.
 */
